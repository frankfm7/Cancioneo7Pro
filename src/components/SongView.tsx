...
export default function SongView({ song: initialSong, onBack, onEdit, songSource }: { 
  song: Song; 
  onBack: () => void; 
  onEdit: () => void;
  ...
  const handleDeleteSong = () => {
    if (confirm(`¿Estás seguro de eliminar "${song.title}"?`)) {
  ...
  const transposeLyrics = (lyrics: string, semitones: number): string => {
  ...
  const transposeChordLine = (chordLine: string, semitones: number): string => {
